...
class DeepSpeedZeroOptimizer_Stage3(ZeROOptimizer):
    ...
    def __init__(
        ...
        self.existing_shm = shared_memory.SharedMemory(name = _name)                    
        ...
        self.zero3_all_reduce_time_array = []
        self.zero3_in_memory_ckpt_time_array = []
        self.zero3_in_memory_copy_time_array = []

        self.parameter_numel = None
    ...
    def destroy(self):
        ...
        del self.__ipg_parameter_bucket_flat_buffer
        
        
        del self.__ipg_parameter_bucket_flat_buffer_cpu
    ...
    def _setup_for_real_optimizer(self):
        ...
        if self.contiguous_gradients:
            self.__ipg_bucket_flat_buffer: Tensor = torch.empty(self.reduce_bucket_size,
                                                                dtype=self.dtype,
                                                                device=get_accelerator().current_device_name())
            # parameter bucket initialization
            self.__ipg_parameter_bucket_flat_buffer: Tensor = torch.empty(self.reduce_bucket_size,
                                                                dtype=self.dtype,
                                                                device=get_accelerator().current_device_name())
            
            self.__ipg_parameter_bucket_flat_buffer_cpu: Tensor = torch.empty(self.reduce_bucket_size,
                                                                dtype=self.dtype,
                                                                device='cpu')
    ...
    def independent_gradient_partition_epilogue(self):
        ...
        # 
        # print('independent_gradient_partition_epilogue')
    ...
    def reduce_independent_p_g_buckets_and_remove_grads(self, param):
        ...
        if self.elements_in_ipg_bucket + param.ds_numel > self.reduce_bucket_size and self.elements_in_ipg_bucket > 0:
            ...
            # print('self.__reduce_and_partition_ipg_grads()')
    ...
    def __add_grad_to_ipg_bucket(self, param: Parameter) -> None:
        ...
        if self.contiguous_gradients and self.elements_in_ipg_bucket + param.grad.numel() <= self.reduce_bucket_size:
            ...
            with get_accelerator().stream(self.reduce_and_partition_stream):
                ...
                # parameter
                new_parameter_tensor = self.__ipg_parameter_bucket_flat_buffer.narrow(0, self.elements_in_ipg_bucket,
                                                                       param.data.numel()).view_as(param.data)
                new_parameter_tensor.copy_(param.data, non_blocking=True)
                ...
                # parameter
                param.data = new_parameter_tensor
    ...
    def __reduce_and_partition_ipg_grads(self, safe_mode: bool = False) -> None:
        ...
        def save_ckpt_to_memory(parameter_bucket):
            ...
            return self.__ipg_parameter_bucket_flat_buffer_cpu[:parameter_bucket.numel()]
        ...
        def print_optimizer_state(optimizer):
            ...
            # self.optimizer.state[fp32_param][optim_state_key]
            
            # key=parameter
            # print('self.optimizer.state.keys() = ', self.optimizer.state.keys())
            
            # print('self.optimizer.state.[0] = ', self.optimizer.state[0])
            
            # print('self.optimizer.state.items()= ', self.optimizer.state.items())
            
            # print('len(self.optimizer.state.items()) = ', len(self.optimizer.state.items()))

            # print('self.optimizer.state[fp32_param] = ', self.optimizer.state[fp32_param])
            ...
            for key, value in self.optimizer.state.items():
                ...
                if key!=[] and value!={}:
                    ...
                    # ResNet-152
                    # key =  tensor([ 1.5746,  4.3225,  2.9711,  ..., -0.0114,  0.0044, -0.0145], device='cuda:0', requires_grad=True)
                    # # value =  {'momentum_buffer': tensor([-0.3423, -0.9470, -0.6630,  ..., -0.0011, -0.0164,  0.0028], device='cuda:0')}

                    # key.numel() =  15048202
                    # value[momentum_buffer].numel() 15048202
                    # print('key.numel() = ', key.numel())
                    # print('value[momentum_buffer].numel()',  value['momentum_buffer'].numel())

                    # BERT+GPT+ViT
                    # key =  tensor([ 0.0135,  0.0035,  0.0225,  ..., -0.0018, -0.1448, -0.0519], device='cuda:0', requires_grad=True)
                    # value =  {'step': 3, 'exp_avg': tensor([-6.3099e-06, -8.2084e-06,  1.2148e-05,  ..., -4.0725e-04, 4.8128e-04, -6.5415e-04], device='cuda:0'), 'exp_avg_sq': tensor([3.4935e-12, 3.1306e-12, 5.9832e-12,  ..., 7.1066e-09, 9.4510e-09, 1.7386e-08], device='cuda:0')}
                    # len(self.optimizer.state.items()) =  2
                    # key.numel() =  88637952
                    # value[exp_avg].numel()  88637952
                    # value[exp_avg_sq].numel()  88637952
                    # key.numel() =  67840
                    # value[exp_avg].numel()  67840
                    # value[exp_avg_sq].numel()  67840
                    # print('key.numel() = ', key.numel())
                    # print('value[exp_avg].numel()',  value['exp_avg'].numel())
                    # print('value[exp_avg_sq].numel()',  value['exp_avg_sq'].numel())
                    
                    # print('self.parameter_numel = ', self.parameter_numel)
                    pass
                    # 
            ...
            # print('self.optimizer.state = ', self.optimizer.state)
            # print('len(self.optimizer.param_groups) = ', len(self.optimizer.param_groups))
        ...
        with get_accelerator().stream(self.reduce_and_partition_stream):
            ...
            # print('reduce_and_partition_stream')
            # print('contiguous_gradients = ', self.contiguous_gradients )
            # print('self.elements_in_ipg_bucket <= self.reduce_bucket_size = ', self.elements_in_ipg_bucket <= self.reduce_bucket_size)
            # print('not self.reduce_scatter = ', not self.reduce_scatter)
            ...
            if self.contiguous_gradients and self.elements_in_ipg_bucket <= self.reduce_bucket_size and not self.reduce_scatter:
                ...
                self.zero3_all_reduce_time_array.append(time.time() - s_time)                
