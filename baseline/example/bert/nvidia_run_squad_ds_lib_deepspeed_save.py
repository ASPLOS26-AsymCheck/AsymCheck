...
def main():
    ...
    # create shared memory buffer for saving the checkpoint in CPU
    # if dist.get_rank() == 0:
    ...
    if args.do_train:
        ...
        ave_rounds = max(10, args.ckpt_freq)
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if (step + 1) % args.ckpt_freq == 0:
                    state = {
                        'epoch': num_epoch + 1,
                        'iteration': step + 1,
                        'arch': args.bert_model,
                        # 'state_dict': model.state_dict(),
                        # 'best_acc1': best_acc1,
                        # 'optimizer' : optimizer.state_dict(),
                        # 'scheduler' : scheduler.state_dict()
                    }
                    delaycheck_lib.utils.save_checkpoint_iteration_deepspeed(model, state, num_epoch + 1,  step)
