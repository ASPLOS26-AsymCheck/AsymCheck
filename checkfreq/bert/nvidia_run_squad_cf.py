...
def main():
    ...
    if args.do_train:
        ...
        if args.cf_iterator:
            train_dataloader = CFIterator(iter(train_dataloader), worker_id=args.local_rank, bs=args.train_batch_size, steps_this_epoch=int(args.start_index/args.train_batch_size), epoch=args.start_epoch, dali=args.dali, cf_manager=cf_manager, chk_freq=args.chk_freq, arch=args.arch, steps_to_run=args.steps_per_run, persist=args.persist, dynamic=args.dynamic, adaptive_tune=args.adaptive_tune,baseline=args.chk_mode_baseline, IOpipeline=args.chk_mode_IOpipeline, fullpipeline=args.chk_mode_fullpipeline, epoch_size=len(train_dataloader))
        ...
        sample_count = 0
        num_epoch = 0
        train_loader_len =  int(math.ceil(train_dataloader._size / args.train_batch_size))
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            num_epoch += 1
            epoch_step = 0
            # for step, batch in enumerate(
            #         tqdm(train_dataloader, desc="Iteration", smoothing=0)):
            for _, (batch, step) in enumerate(train_dataloader):
                if n_gpu == 1:
                    batch = tuple(
                        t.to(device)
                ...
                if args.local_rank == 0 and step % args.print_freq == 0 and step > 1:
                    print('Epoch: [{0}][{1}/{2}]\t'
                        .format(
                        num_epoch, step, train_loader_len))
