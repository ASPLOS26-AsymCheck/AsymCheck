...
sys.path.append('./')
sys.path.append('./src')
from cf_checkpoint_0730 import CFCheckpoint
from cf_manager_0730 import CFManager, CFMode
from cf_iterator_0730 import CFIterator
import time
...
def main():
    ...
    if args.dataset_name is not None:
        ...
        #     data_files["val"] = os.path.join(args.validation_dir, "**")
    ...
    print('----------------------------------------------')
    print('train in dataset = ', "train" in dataset)
    ...
    if args.cf_iterator:
        train_dataloader = CFIterator(iter(train_dataloader), worker_id=args.local_rank, bs=args.per_device_train_batch_size, steps_this_epoch=int(args.start_index/args.per_device_train_batch_size), epoch=args.start_epoch, dali=args.dali, cf_manager=cf_manager, chk_freq=args.chk_freq, arch=args.arch, steps_to_run=args.steps_per_run, persist=args.persist, dynamic=args.dynamic, adaptive_tune=args.adaptive_tune,baseline=args.chk_mode_baseline, IOpipeline=args.chk_mode_IOpipeline, fullpipeline=args.chk_mode_fullpipeline, epoch_size=len(train_dataloader))
    ...
    train_loader_len =  int(math.ceil(train_dataloader._size / args.per_device_train_batch_size))
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for _, (batch, step)in enumerate(active_dataloader):
            ...
            if args.local_rank == 0 and step % args.print_freq == 0 and step > 1:
                print('Epoch: [{0}][{1}/{2}]\t'
                    .format(
                        args.num_train_epochs, step, train_loader_len))
