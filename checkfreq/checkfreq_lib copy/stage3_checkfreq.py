...
def save_ckpt_to_disk(queue, rank, save_dir):
    while True:
        cpu_tensor_array = queue.get()
        print("start to save ckpt to disk")
        sys.stdout.flush()
        torch.save(cpu_tensor_array, save_dir + "ckpt_" + "rank" + str(rank) + ".pt")
        
def save_ckpt_to_disk_sync(cpu_tensor_array, rank, save_dir):
...
class DeepSpeedZeroOptimizer_Stage3(ZeROOptimizer):
    ...
    def __init__(
        ...
        self.save_ckpt_in_memory_thread = None
        ...
        self.queue = mp.Queue()
        self.cpu_tensor_dict= {}
        self.step_count = -1
        # self.save_ckpt_to_disk_process = mp.Process(target=save_ckpt_to_disk, args=(self.queue, dist.get_rank(), "./checkpoint/"))
    ...
    def save_ckpt_in_memory(self, rank, module_state_dict, optimizer_state_dict):
        ...
        while True:
            with torch.cuda.stream(stream):
                for key, tensor in module_state_dict.items():
                    ...
                    cpu_tensor_dict[key + str(id)] = cpu_tensor
            id += 1
            stream.synchronize()
        ...
        with torch.cuda.stream(stream):
            for tensor, momentum in optimizer_state_dict.items():
                ...
                cpu_tensor_dict['exp_avg' + str(id)] = cpu_tensor
            
                numel+=cpu_tensor.numel()
     
                cpu_tensor = momentum['exp_avg_sq'].to('cpu', non_blocking=True)
                cpu_tensor_dict['exp_avg_sq' + str(id)] = cpu_tensor
                numel+=cpu_tensor.numel()
                id += 1
        stream.synchronize()
        ...
        self.cpu_tensor_dict = cpu_tensor_dict
    ...
    def _setup_for_real_optimizer(self):
        ...
        # parameter bucket initialization
            # Add an additional buffer to save parameters
    ...
    def __reduce_and_partition_ipg_grads(self, safe_mode: bool = False) -> None:
        ...
        with get_accelerator().stream(self.reduce_and_partition_stream):
            ...
            if self.contiguous_gradients and self.elements_in_ipg_bucket <= self.reduce_bucket_size and not self.reduce_scatter:
                ...
                # self.process_model = threading.Thread(target=self.model_copy_async, args=(rank, self.__ipg_parameter_bucket_flat_buffer,))
    ...
    def step(self, closure=None):
        if self.step_count == 40:
            save_ckpt_to_disk_sync(self.cpu_tensor_dict, dist.get_rank(), "./checkpoint/")
        
        """
            Not supporting closure.
        """
        # if self.save_ckpt_in_memory_thread != None:
        #     self.save_ckpt_in_memory_thread.join()
    ...
    def backward(self, loss, retain_graph=False):
        ...
        if dist.get_rank() == 0:
            print('before backward', time.time() - s_time)
        ...
        if dist.get_rank() == 0:
            print('loss backward', time.time() - s_time)
        ...
        if dist.get_rank() == 0:
            print('end backward', time.time() - s_time)
