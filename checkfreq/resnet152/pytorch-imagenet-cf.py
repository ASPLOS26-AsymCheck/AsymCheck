...
class DistributedSampler(Sampler[T_co]):
    def __init__(self, dataset: Dataset, num_replicas: Optional[int] = None,
                 ...
                 seed: int = 0, drop_last: bool = False, recovery_iters: int = 0, batch_size: int = 32) -> None:
    ...
    def __iter__(self) -> Iterator[T_co]:
        if self.shuffle:
            ...
            indices = torch.randperm(len(self.dataset), generator=g).tolist()[: -1]  # type: ignore[arg-type]
        else:
            indices = list(range(len(self.dataset)))[: -1]  # type: ignore[arg-type]
        ...
        indices = indices[self.recovery_iters * self.batch_size:-1]
        return iter(indices)
...
def main():
    ...
    if args.chk_mode_baseline or args.chk_mode_sync or args.chk_mode_IOpipeline or args.chk_mode_fullpipeline:
        args.chk_mode = CFMode.MANUAL
    else:
        args.chk_mode = CFMode.AUTO

    chk = CFCheckpoint(model=model, optimizer=optimizer)
    cf_manager = CFManager(args.chk_prefix, chk, mode=args.chk_mode, worker_id=args.local_rank, chk_mode_IOpipeline=args.chk_mode_IOpipeline, chk_GPU_stall= (args.chk_mode == CFMode.AUTO), chk_fullpipeline_stall=args.chk_mode_fullpipeline)
    #else:
    #    cf_manager = None
    ...
    if args.recovery:
        with open("recovery_time.csv", 'w+') as fp_recovery:
                    fp_recovery.write("load_time, recovery_time\n")
    ...
    #     if args.cf_iterator:
    #         train_loader = CFIterator(train_loader, worker_id=args.local_rank, bs=args.batch_size, \
    #             steps_this_epoch=int(args.start_index/args.batch_size), epoch=args.start_epoch, dali=args.dali, \
    #                 cf_manager=cf_manager, chk_freq=args.chk_freq, arch=args.arch, steps_to_run=args.steps_per_run, \
    #                     persist=args.persist, dynamic=args.dynamic, adaptive_tune=args.adaptive_tune,baseline=args.chk_mode_baseline, \
    #                         IOpipeline=args.chk_mode_IOpipeline, fullpipeline=args.chk_mode_fullpipeline, max_overhead=args.max_overhead)
    #         if args.resume:
    ...
    #         val_loader = DALIClassificationIterator(pipe_val, size=int(pipe_val.epoch_size("Reader") / args.world_size))
        ...
        if args.cf_iterator:
            train_loader = CFIterator(iter(train_loader), worker_id=args.local_rank, bs=args.batch_size, steps_this_epoch=int(args.start_index/args.batch_size), epoch=args.start_epoch, dali=args.dali, cf_manager=cf_manager, chk_freq=args.chk_freq, arch=args.arch, steps_to_run=args.steps_per_run, persist=args.persist, dynamic=args.dynamic, adaptive_tune=args.adaptive_tune,baseline=args.chk_mode_baseline, IOpipeline=args.chk_mode_IOpipeline, fullpipeline=args.chk_mode_fullpipeline, epoch_size=len(train_loader))
            if args.resume:
                train_loader.load_state_dict(extra_state)
    ...
    with open("epoch_time.csv", 'w+') as fp_epoch_time:
        fp_epoch_time.write("epoch_time\n")
    for epoch in range(args.start_epoch, args.epochs):
        ...
        avg_train_time, epoch_time = train(train_loader, model, criterion, optimizer, epoch, df, cf_manager, train_dataset, stop_iteration)
        ...
        train_loader._steps_this_epoch = 0
...
def train(train_loader, model, criterion, optimizer, epoch, df, cf_manager, dataset, stop_iteration):
    ...
    load_time = 0
    in_recovery = False
    num_iterations = int(train_loader._size / train_loader._batch_size)
    recovery_iter_interval = None
    iter_since_recovery = 0
    device = torch.device(f'cuda:{args.local_rank}')
    recovery_iter_interval_tensor =  torch.tensor([0], dtype=torch.int).to(device)

    for _, (data, i) in enumerate(train_loader):
        if i == train_loader._max_steps_per_epoch - 1 : 
            break
        if i == 150 :
            if args.local_rank == 0 and train_loader._profile_done:
                recovery_iter_interval = int(args.recovery_interval / train_loader._avg_iter_dur)
                recovery_iter_interval_tensor = torch.tensor([recovery_iter_interval], dtype=torch.int).to(device)
                iter_since_recovery = 0
            dist.broadcast(recovery_iter_interval_tensor, src=0)
            recovery_iter_interval = recovery_iter_interval_tensor.item()
            iter_since_recovery = 0
        iter_since_recovery += 1
        if args.local_rank == 0 and train_loader._steps_this_epoch ==  recovery_iters and in_recovery and args.recovery:
            with open("recovery_time.csv", 'a+') as fp_recovery:
                fp_recovery.write("{},{}\n".format(load_time - last_recovery_time, load_time - last_recovery_time + time.time() - start_again_time))
            in_recovery = False
             
        if  iter_since_recovery == recovery_iter_interval and args.recovery and i > 150:
            in_recovery = True
            last_recovery_time = time.time()
            ...
            #     #train_sampler = DistributedSampler(dataset, recovery_iters=train_loader._steps_this_epoch)
            # else:
            #     train_sampler = None
            ...
            # if extra_state is not None:
            #     recovery_index = extra_state['iter']
            
            train_loader.load_state_dict(extra_state)
            print("valid iterations:{}".format(train_loader._total_steps))
                    
            torch.distributed.barrier()
            iter_since_recovery = 0
            if args.local_rank == 0:
                start_again_time = time.time()
    ...
    return batch_time.avg, batch_time.sum
