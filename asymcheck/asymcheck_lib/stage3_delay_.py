...
class DeepSpeedZeroOptimizer_Stage3(ZeROOptimizer):
    ...
    def __init__(
        ...
        self.exp_avg_numel =0
        self.exp_avg_sq_numel =0
        ...
        # self.cuda_stream_model_array = []
        self.cuda_stream_model_dict ={}
        ...
        see_memory_usage("DeepSpeedZeRO snapshot initialize [end]", force=True)
    ...
    # Modify by authors
    # 20250823
    def flat_tensor_model_parameters(model):
        all_tensors = []
        param_shapes = []
        ...
        flat_tensor = torch.cat(all_tensors, dim=0)
        
        return flat_tensor, param_shapes
    
    def partition_flat_tensor(flat_tensor, forward_idle_periods, backward_idle_periods):
        all_periods = forward_idle_periods + backward_idle_periods
        total_length = sum(all_periods)
        partitions_cpu = [torch.zeros(length, device='cpu') for length in all_periods]
         
        return partitions_cpu
            ...
            if not hasattr(module, "pre_bwd_fn"):
                ...
                class PreBackwardFunctionForModule(torch.autograd.Function):
                    ...
                    def forward(ctx, outputs):
                        ...
                        self.forward_idle_periods = self._get_param_coordinator(training=True).idle_periods
                        return outputs
    ...
    def pre_sub_module_forward_function(self, sub_module):
        ...
        param_coordinator.start_time = start_time
    ...
    def step(self, closure=None):
        ...
        # process.join()
        # if dist.get_rank() == 0:
        #     print("optimizer block time = ", time.time() - block_time)
    ...
    def test_module_parameter(self, model):
        total_size = 0
        array_value = []
        ...
        if dist.get_rank() == 0:
            ...
            # print('module value = ', array_value[0])
            # print('module value = ', array_value[-1])
        
        pass


    def first_second_copy_optimizer(self, optimizer):
        if optimizer.state == {}:
            return

        if self.gpu_optimizer_buffer_1 == None or self.gpu_optimizer_buffer_2 == None:
        ...
        self.optimizer_stream_1.synchronize()
        
        rank  = dist.get_rank()
        for tensor, momentum in self.optimizer.state.items():
        
            
            # optimizer_buffer_name_1 = 'optimizer_buffer_1'
            # existing_optimizer_shm_1 = shared_memory.SharedMemory(name = optimizer_buffer_name_1)
            # shape = (4, 1024*1024*1024*10)
            # optimizer_shard_buffer_1 = np.ndarray(shape, dtype=np.float16, buffer=existing_optimizer_shm_1.buf)

            # with torch.cuda.stream(self.optimizer_stream_1):
            with torch.cuda.stream(torch.cuda.Stream()):
                start = sum(self.gpu_optimizer_exp_avg_elements_array)
                exp_avg_numel = momentum['exp_avg_sq'].numel()

                # print('self.gpu_optimizer_buffer_1.numel() = ', self.gpu_optimizer_buffer_1.numel())
                # print('exp_avg_numel = ', exp_avg_numel)
                # parameter_tensor_cpu = self.gpu_optimizer_buffer_1[start:start+exp_avg_numel].copy_(momentum['exp_avg'], non_blocking=True).to('cpu', non_blocking=True)
                
                parameter_tensor_cpu = momentum['exp_avg'].to('cpu', non_blocking=True)
                
                # optimizer_shard_buffer_1[rank, start : start + exp_avg_numel] = parameter_tensor_cpu.numpy()
                self.gpu_optimizer_exp_avg_elements_array.append(exp_avg_numel)

        # self.optimizer_stream_2.synchronize()
        for tensor, momentum in self.optimizer.state.items():
           

            # optimizer_buffer_name_2 = 'optimizer_buffer_2'
            # existing_optimizer_shm_2 = shared_memory.SharedMemory(name = optimizer_buffer_name_2)
            # shape = (4, 1024*1024*1024*10)
            # optimizer_shard_buffer_2 = np.ndarray(shape, dtype=np.float16, buffer=existing_optimizer_shm_2.buf)
            
            # with torch.cuda.stream(self.optimizer_stream_2):
            with torch.cuda.stream(torch.cuda.Stream()): 
                start = sum(self.gpu_optimizer_exp_avg_sq_elements_array)
                exp_avg_numel = momentum['exp_avg_sq'].numel()
                ...
                # optimizer_shard_buffer_2[rank, start : start + exp_avg_numel] = parameter_tensor_cpu.numpy()

                self.gpu_optimizer_exp_avg_sq_elements_array.append(exp_avg_numel)

        pass

   
    def first_second_copy_optimizer_async(self, optimizer):
    ...
    def copy_model(self, model):
        ...
        #             tensor_numel = value.numel()
        #             self.gpu_buffer_state[key] = torch.empty(tensor_numel, dtype=torch.float16, device=self.device)
        ...
        # 
        # Modify 
        # 每个元素4字节 (float32类型)，32MB = 32 * 1024 * 1024字节
        # buffer_size = 32 * 1024 * 1024 // 4  # 元素数
        buffer_size = 1024 * 1024 * 1024 // 2  # 元素数
        # 
        # 分配一个32MB的GPU缓冲区
        ...
        # with torch.cuda.stream(torch.cuda.Stream()):
        #     ramdisk_path = "/mnt//model/model_"+str(rank)+".pt"
        #     torch.save(model.state_dict(), ramdisk_path)
        # return
    ...
    # 
    def optimizer_copy_to_shared_memory_thread_1(self):
    ...
    def optimizer_copy_to_shared_memory_thread_2(self):
    ...
    def model_copy_to_shared_memory_thread(delf):
    ...
    def backward(self, loss, retain_graph=False):
        ...
        # if dist.get_rank() == 0:
        #     print('first_copy_optimizer =', time.time() - s_time)
        ...
        # if dist.get_rank() == 0:
        #     print('copy_model =', time.time() - s_time)
        ...
        self.backward_idle_periods = self._get_param_coordinator(training=True).idle_periods
        ...
        # process.join()
        # if dist.get_rank() == 0:
        #     print("model block time = ", time.time() - block_time)
