...
parser.add_argument("--load_freq",
                    type=int,
                    default=50,
                    help="Recovery from checkpoint frequency")
...
def train(train_loader, model, criterion, optimizer, epoch, device, args):
    ...
    if dist.get_rank() == 0:
        with tqdm(total=len(train_loader), mininterval = 10,
              desc='Train Epoch   #{}'.format(epoch + 1),
              disable=False, leave=False) as t: 
            ...
            for idx, (images, target) in enumerate(train_loader):
                ...
                if numel_count == 0 and len(optimizer.state.items()) != 0:
                    ...
                    for key, value in optimizer.state.items():
                        numel_dict['momentum'] = [numel_count, numel_count + value['momentum_buffer'].numel()]
                ...
                if dist.get_rank() % torch.cuda.device_count() == 0 and idx % args.ckpt_freq == 0 and numel_count != 0:
                    ...
                    asymcheck_lib.utils.save_checkpoint_async_model(model, model_state, epoch, idx)
                ...
                if idx % args.print_freq == 0 and dist.get_rank()==0:
                    ...
                    # print('per_iteration_io = ', sum(iteration_io_time_array)/args.print_freq)
                    print('per_iteration_forward_time = ', sum(iteration_forward_time_array)/args.print_freq)
                    per_iteration_backward_allreduce_time = sum(iteration_backward_time_array)/args.print_freq
                    print('per_iteration_backward_allreduce_time = ', per_iteration_backward_allreduce_time)
                    print('per_iteration_step_time = ', sum(iteration_step_time_array)/args.print_freq)

                    print('------------------------------------------------------------------------')
                    per_backward_time = backward_time/args.print_freq
                    print('per_backward_time = ', per_backward_time)
                    # print('per_allreduce_time = ', allreduce_time/args.print_freq)
                ...
                # t.set_postfix({'loss': losses,'accuracy': top1})
                # t.update(1)
                
                progress_save = torchsnapshot.StateDict(current_epoch=0)
                # torchsnapshot: define app state
                app_state: Dict[str, Stateful] = {
                        "rng_state": torchsnapshot.RNGState(),
                        "model": model,
                        "optim": optimizer,
                        "progress": progress_save,
                    }
        ...
        for i, (images, target) in enumerate(train_loader):
            ...
            # save_checkpoint_in_disk_snapshot(progress_save, app_state, checkpoint_save_work_dir)
            if numel_count == 0 and len(optimizer.state.items()) != 0:
            ...
            if dist.get_rank() % torch.cuda.device_count() == 0 and i % args.ckpt_freq == 0 and numel_count != 0:
                model_state =  optimizer.shard_buffer[:numel_count]
                asymcheck_lib.utils.save_checkpoint_async_model(model, model_state, epoch, i)
