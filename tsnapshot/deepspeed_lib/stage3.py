        ...
        with get_accelerator().stream(self.reduce_and_partition_stream):
            ...
            if self.contiguous_gradients and self.elements_in_ipg_bucket <= self.reduce_bucket_size and not self.reduce_scatter:
                ...
                if 1 == 0:
                    ...
                    # Epoch time =  197.9122440814972
                    # Epoch time =  167.8206181526184

                    # 
                    # 这个动作比较耗时, 2机四卡训练, 
                    # 
                    # if dist.get_rank() ==0:
                    #     if self.parameter_cpu is None:
                    #         self.parameter_cpu = parameter_numpy_cpu
                    #     else:
                    #         self.parameter_cpu  = np.concatenate((self.parameter_cpu, parameter_numpy_cpu))
                    # # print('parameter_numpy_pu = ', parameter_numpy_cpu, 'parameter_numel = ', parameter_numel)
                    # 
                    # 
