...
def main():
    ...
    print(f"Going to initalize datastates engine...", dist.get_rank())
    ...
    config.max_position_embeddings = 2048  # Example value
    model = AutoModelForMaskedLM.from_config(config, trust_remote_code=args.trust_remote_code)
    ...
    print('preprocessing_num_workers = ', args.preprocessing_num_workers)
    ...
    import datastates_lib as datastates_lib
    
    model, optimizer, _, _ = datastates_lib.initialize(
        args=args,
        model=model,
            ...
            for step, batch in enumerate(active_dataloader):
                ...
                if step == 40:
                    ...
                    if dist.get_rank() == 0:
                        ...
                        print("Start sync ckpt to disk")
                        print(datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-2])
                    ckpt_obj = {
                        "ckpt": optimizer.cpu_tensor_dict,
                    }
                    ckpt_path = "./checkpoint/" + "ckpt_rank" +  str(dist.get_rank()) + ".pt"
                
                    ckpt_engine.save(state_dict=ckpt_obj, path=ckpt_path)
                    ckpt_engine.wait()
                    if dist.get_rank() == 0:
                        print("sync ckpt to disk time = ", time.time() - ckpt_time)
