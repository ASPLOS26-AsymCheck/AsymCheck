...
CMD=" deepspeed --num_nodes ${NUM_NODES} --num_gpus ${NGPU_PER_NODE} \
      ...
      --hostfile ${HOSTFILE} \
      ../run_clm_no_trainer_deepspeed_layer_10B_fastpersist.py   \
      --job_name ${JOB_NAME} \
      --fp16 \
      --deepspeed \
...
CMD+=" --dataset_name wikitext-2-raw-v1 --dataset_config_name default "
CMD+=" --model_name_or_path gpt2 "
CMD+=" --output_dir  ../checkpoint/ "
