    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        with tqdm(total=len(train_dataloader),
            ...
            for step, batch in enumerate(active_dataloader):
                ...
                if completed_steps and completed_steps % args.load_freq == 0 and numel_count != 0:
...
#     if args.do_train and torch.distributed.get_rank()==0 and not args.skip_checkpoint:
...
def save_checkpoint(epoch,iteration,filepath):
    if True:
    # if torch.distributed.get_rank() == 0:
        # filepath = args.output_dir.format(epoch=epoch + 1, iteration = iteration + 1)
        state = {
            'model': model.state_dict(),
            'optimizer': optimizer.state_dict(),
            }
...
if __name__ == "__main__":
    ...
    if (dist.get_rank()) % torch.cuda.device_count() == 0:
        ...
        try:
            shm = shared_memory.SharedMemory(name=_name_parameter)
            shm.unlink() 
        ...
        try:
            shm = shared_memory.SharedMemory(name=_name_parameter_backup)
            shm.unlink() 
        except FileNotFoundError:
            pass
        ...
        try:
            shm = shared_memory.SharedMemory(name=_name_optimizer)
            shm.unlink() 
        except FileNotFoundError:
            pass
        ...
        try:
            shm = shared_memory.SharedMemory(name=_name_optimizer_backup)
            shm.unlink() 
        except FileNotFoundError:
            pass
