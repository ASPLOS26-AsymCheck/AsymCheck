...
class CFCheckpoint:
	...
	#	snap_ptr = {}
		...
		#for name, ref in self.tracking_map.items():
			...
			# 	snapshot[name] = _to_cpu(ref.state_dict())
	...
	def persist_async(self,snapshot, filepath, iter_chk, overwrite, linkpath, epoch_chk, lock, active_snapshot):
		torch.save(snapshot, filepath)
		with lock:
			active_snapshot.value = 0
	...
	def _snapshot_and_persist_async_IOpipeline(
		self,
		filepath, 
		active_snapshot,
		in_progress_snapshot,
	...
	#	snap_ptr = {}
		...
		if profile:
			with lock:
				active_snapshot.value = 0
			return
		fn = threading.Thread(target=self.persist_async,	\
						args=[snapshot, filepath, iter_chk, overwrite, linkpath, epoch_chk, lock, active_snapshot])
		fn.start()
